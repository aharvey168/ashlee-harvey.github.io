...
var arraySum = function(array) {
  ...
  //recursion
  return(arraySum(array, array.length - 1) + array[array.length - 1])
...
var range = function(start, end, output=[]) {
  ...
  if(start === end){
    return [];
  } else if(start === end + 1){ //to exclude the start and end values
    return output; 
  } else if(start === end - 1){
    return output;
  }
  //recursion
  if(start > end){
      output = range(start - 1, end);
      output.unshift(start - 1);
      return output;
  } else {
    output = range(start + 1, end);
    output.unshift(start + 1);
